...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d'):
        ...
        # uv coords
        if uvcoords.shape == torch.Size([1, 0]):
            uvcoords = torch.zeros(1, faces.shape[1], 2)
            uvfaces = faces.clone()
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
        ...
        # TODO: test
        # if vertices.shape[1] == 6890:
        #     reg = load_obj('/workspace/data/temp/singlemesh.obj')
        #     self.faces = reg[2].unsqueeze(0).to('cuda:0')
        #     self.face_colors = torch.ones(self.faces.shape[1], 3, 3, dtype=self.face_colors.dtype, device=self.face_colors.device) * 0.7059
        #     self.face_uvcoords = torch.zeros(self.faces.shape[1], 3, 3, dtype=self.face_colors.dtype, device=self.face_colors.device)
